/**
 * Qwen3-VL-8B-Instruct Analysis Functions for Sinna 1.0
 * 
...
import { qwenInstruct, logQwenUsage, QwenPromptPayload, QwenResponse } from '../lib/qwenClient';
...
/**
 * Generate accessibility metadata for compliance
 */
export async function generateAccessibilityMetadata(
  presetId: string,
  hasCaptions: boolean,
  ...
  const prompt: QwenPromptPayload = {
    messages: [
      {
        role: 'system',
        content: 'You are an accessibility compliance expert. Generate metadata for WCAG compliance.',
      },
      {
        role: 'user',
        content: `Generate accessibility metadata for preset: ${presetId}
- Captions: ${hasCaptions}
- Audio Description: ${hasAudioDescription}
- Contrast Safe: ${contrastSafe}
- Cognitive Score: ${cognitiveScore || 'N/A'}

Return JSON with compliance checklist.`,
      },
    ],
    response_format: { type: 'json_object' },
    temperature: 0.2,
  ...
  try {
    ...
    return {
      ...
      contrastSafe,
      cognitiveScore: cognitiveScore || 0.8,
      preset: presetId,
      compliance: metadata.compliance || ['WCAG 2.1 AA'],
    };
  } catch (error) {
