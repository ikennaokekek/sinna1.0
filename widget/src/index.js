/**
 * Sinna Preset Widget - Premium Web Component
 * 
 * Usage:
 * <script src="https://cdn.sinna.site/widget.js"></script>
 * <sinna-preset 
 *   api-key="sk_live_..." 
...
// ResultsViewer component should be loaded before this file
// It registers globally as 'sinna-results-viewer'

class SinnaPreset extends HTMLElement {
  constructor() {
    super();
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if (oldValue !== newValue) {
      if (name === 'api-key') {
        ...
        this.updateTheme();
        // Re-render to apply accent changes
        if (this.shadowRoot && this.shadowRoot.innerHTML) {
          this.render();
        }
      }
  ...
  updateTheme() {
    // Set theme data attribute on host element
    if (this.theme) {
    ...
    // Update CSS variables in shadow root dynamically
    ...
    // Update ResultsViewer if it exists
  ...
  adjustBrightness(hex, percent) {
    // Simple brightness adjustment for accent colors
  ...
  showResults(data) {
    ...
    if (resultArea) {
      ...
      // Pass theme and accent to viewer
      if (this.theme) viewer.setAttribute('data-theme', this.theme);
      if (this.accent) viewer.setAttribute('data-accent', this.accent);
      viewer.setData(data);
      resultArea.innerHTML = '';
...
customElements.define('sinna-preset', SinnaPreset);

// Export for ES modules
export default SinnaPreset;
